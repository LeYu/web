...
public class YZMrender extends Render
{
    ...
    public YZMrender(String randomKey, int width, int height, int count, boolean isCaseInsensitive) {
        if (StrKit.isBlank(randomKey))
            throw new IllegalArgumentException("randomKey can not be blank");
        randomCodeKey = randomKey;
    ...
    private String drawGraphic(BufferedImage image){
        ...
        for (int i = 0; i < img_randNumber; i++) {
            ...
            sRand += rand;
            // 将认证码显示到图象中
            int fontSize = random.nextInt(20);
            g.setFont(new Font("Times New Roman", Font.LAYOUT_LEFT_TO_RIGHT, 24));
